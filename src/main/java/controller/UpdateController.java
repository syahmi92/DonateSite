...
public class UpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getPart("item-photo-replace-1").getSize() > 0) {
				
				photo1 = request.getPart("item-photo-replace-1");
				...
				name_photo1 = request.getParameter("first_img_hidden");
			}

			if (request.getPart("item-photo-replace-2").getSize() > 0) {
				
				photo2 = request.getPart("item-photo-replace-2");
				...
				name_photo2 = request.getParameter("second_img_hidden");
			}

			if (request.getPart("item-photo-replace-3").getSize() > 0) {
				
				photo3 = request.getPart("item-photo-replace-3");
				...
				name_photo3 = request.getParameter("third_img_hidden");
			}

			if (request.getPart("item-photo-replace-4").getSize() > 0) {
				photo4 = request.getPart("item-photo-replace-4");
				...
				name_photo4 = request.getParameter("fourth_img_hidden");
			}
		...
		int item_id_origin = Integer.parseInt(request.getParameter("itemID_hidden"));
		...
		item.setItem_id(item_id_origin);
		...
		try {
			if (dbHandler.update_item(item) > 0) {
		...
		RequestDispatcher dispatcher = request.getRequestDispatcher("donor_item_list.jsp");
